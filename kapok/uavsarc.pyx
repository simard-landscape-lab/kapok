...
def load(infile, outfile, mlwin=(20,5), smwin=(1,1), azbounds=None,
    ...
    cdef np.ndarray[np.float64_t, ndim=3] pos = np.array(posmm,dtype='float64')
    del posmm
    
    cdef np.ndarray[np.float64_t, ndim=2] inc_buffer = np.zeros((mlwin[0],rngsize_slc),dtype='float64')
    cdef np.ndarray[np.float64_t, ndim=2] kz_buffer = np.zeros((mlwin[0],rngsize_slc),dtype='float64')
    
    cdef np.ndarray[np.float64_t, ndim=3] tracklkv = np.zeros((mlwin[0],rngsize_slc,3),dtype='float64')
    cdef np.ndarray[np.float64_t, ndim=3] velocity = np.zeros((mlwin[0],rngsize_slc,3),dtype='float64')
    cdef np.ndarray[np.float64_t, ndim=3] lookcrossv = np.zeros((mlwin[0],rngsize_slc,3),dtype='float64')
    cdef np.ndarray[np.float64_t, ndim=3] tempdiff = np.zeros((mlwin[0],rngsize_slc,3),dtype='float64')
    cdef np.ndarray[np.float64_t, ndim=3] proj_lkv_velocity = np.zeros((mlwin[0],rngsize_slc,3),dtype='float64')

    cdef np.ndarray[np.float64_t, ndim=2] base_enu = np.zeros((azbounds[1]-azbounds[0],3),dtype='float64')

    cdef np.ndarray[np.float64_t, ndim=2] baseline = np.zeros((mlwin[0],3),dtype='float64')    
    cdef np.ndarray[np.float64_t, ndim=2] baselinep = np.zeros((mlwin[0],rngsize_slc),dtype='float64')
    cdef np.ndarray[np.float64_t, ndim=2] baselinepsign = np.zeros((mlwin[0],rngsize_slc),dtype='float64')
    
    cdef unsigned long az_ml, lkvrowstart, lkvrowend, lkvtrimstart, lkvtrimend, azposstart, azposend
