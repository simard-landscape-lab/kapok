...
def pdopt(np.ndarray[np.complex64_t, ndim=4] tm,
          np.ndarray[np.complex64_t, ndim=4] om, numph=30, step=50, reg=0.0,
          returnall=False):
    ...
    cdef np.ndarray[np.float32_t, ndim=2] mincohdiff = np.ones(cohsize,dtype='float32') * 99
    cdef np.ndarray[np.complex64_t, ndim=2] gammaminormax = np.zeros(cohsize,dtype='complex64')
    cdef np.ndarray[np.complex64_t, ndim=2] gammaminormin = np.zeros(cohsize,dtype='complex64')
